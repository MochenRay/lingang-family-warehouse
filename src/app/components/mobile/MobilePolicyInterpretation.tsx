...
import { ArrowLeft, Send, Bot, BookOpen, Sparkles, Loader2 } from 'lucide-react';
...
export function MobilePolicyInterpretation({ onBack }: MobilePolicyInterpretationProps) {
  ...
  const handleSendMessage = async (text: string = inputMessage) => {
    ...
    try {
      const response = await secondaryAiRepository.sendMessage('policy', nextPrompt);
      setMessages(prev => [
        ...prev,
        {
          id: timestamp + 1,
          role: 'ai',
          content: response.content || buildSecondaryAiReply('policy', nextPrompt),
        },
      ]);
    } finally {
      setSending(false);
