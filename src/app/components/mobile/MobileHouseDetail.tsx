...
import { ArrowLeft, Home, MapPin, Users, History, Calendar, ChevronRight, Tag, AlertCircle } from 'lucide-react';
...
export function MobileHouseDetail({ id, onBack, onRouteChange }: MobileHouseDetailProps) {
  ...
  useEffect(() => {
    const loadHouse = () => {
      ...
      if (houseData) {
        ...
        // 加载居住人员
        const houseResidents = db.getPeople().filter(p => p.houseId === id);
        setResidents(houseResidents);
        
        // 加载历史记录
        const history = db.getHousingHistory().filter(h => h.houseId === id);
        setHousingHistory(history);
      }
    };

    loadHouse();
    window.addEventListener('db-change', loadHouse);
    return () => window.removeEventListener('db-change', loadHouse);
  }, [id]);
  ...
  // 找出户主
  const owner = residents.find(r => r.name === house.ownerName);
  ...
  return (
    <div className="h-full bg-[var(--color-bg-primary)] flex flex-col overflow-hidden">
            ...
            {house.tags && house.tags.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-2">
                {house.tags.map((tag, i) => (
                  <span key={i} className="text-xs bg-white/10 px-2 py-1 rounded backdrop-blur-md border border-white/10 text-white/90">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          {residents.length > 0 ? (
            <>
              <div className="divide-y divide-[var(--color-border-secondary)]">
                {residents.map((resident, i) => (
                  <div key={i} className="p-4 flex items-center justify-between active:bg-[var(--color-bg-tertiary)] transition-colors">
                    <div className="flex items-center gap-3">
            ...
            <div className="p-4">
              <div className="text-center py-8">
                ...
                <Button variant="ghost" className="mt-3 text-blue-600 text-sm hover:text-blue-700 hover:bg-blue-50">
                  + 添加居住人员
                </Button>
              </div>
            </div>
          ...
          <div className="p-4">
            {housingHistory.length > 0 ? (
              <div className="relative pl-4 space-y-6 before:absolute before:left-[5px] before:top-2 before:bottom-2 before:w-0.5 before:bg-[var(--color-border-secondary)]">
                {housingHistory.map((item, i) => (
                  <div key={i} className="relative">
                    <div className={`absolute -left-[1.35rem] w-3 h-3 rounded-full border-2 border-[var(--color-bg-secondary)] ring-1 ring-[var(--color-border-secondary)] ${ 
                      item.type === '业主' ? 'bg-orange-500' : 
                      item.type === '租客' ? 'bg-blue-500' : 
                      item.type === '家属' ? 'bg-green-500' :
                      'bg-gray-400'
                    }`} />
      ...
      <div className="bg-[var(--color-bg-secondary)] border-t border-[var(--color-border-primary)] p-4 safe-area-bottom sticky bottom-0">
        <Button className="w-full h-11 bg-[var(--color-brand-primary)] hover:bg-[var(--color-brand-hover)] text-base shadow-lg">
          信息采集变更
        </Button>
      </div>
    </div>
