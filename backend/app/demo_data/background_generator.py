...
def _build_recent_migration_histories(
    ...
    for month_index, (month, inbound_count, outbound_count) in enumerate(RECENT_MIGRATION_MONTHS):
        for event_index in range(inbound_count):
            ...
            day = _bounded_migration_day(
                month,
                2 + ((month_index * 5 + event_index * 3) % 24),
            )
            histories.append(
                HousingHistory(
                    id=state.next_history_id(),
        ...
        for event_index in range(outbound_count):
            ...
            day = _bounded_migration_day(
                month,
                3 + ((month_index * 7 + event_index * 4) % 23),
            )
            histories.append(
                HousingHistory(
                    id=state.next_history_id(),
...
def _bounded_migration_day(month: str, day: int) -> int:
    year, month_number = (int(part) for part in month.split("-", maxsplit=1))
    if (year, month_number) == (
        MIGRATION_REFERENCE_DATE.year,
        MIGRATION_REFERENCE_DATE.month,
    ):
        return min(day, MIGRATION_REFERENCE_DATE.day)
    return day
